...
from beam_position.main_window import BeamPosition, APP_NAME
...
def excepthook(exc_type, exc_value, traceback_obj):
    ...
    """
    separator = '-' * 80
    log_path = f"{os.path.expanduser('~')}/.beam_position/error.log"
...
def setup_logger(args):
    ...
    if not os.path.exists(f"{str(Path.home())}/.beam_position"):
        os.mkdir(f"{str(Path.home())}/.beam_position")

    filename = f"{str(Path.home())}/.beam_position/beam_position.log"
...
def main():
    ...
    parser.add_option("-b", "--beamline", dest='beamline', default='p23', help="Beamline number")
    ...
    mainWindow = BeamPosition(options)
    mainWindow.show()
